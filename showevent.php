...
<html lang="en">
  ...
  echo '<div class="card-body">';
      echo ' <h5 class="card-title">Title: '.$row['title'].'</h5>';
        echo ' <h5 class="card-title">Description: '.$row['description'].'</h5>';
      echo ' <h5 class="card-title">Address: '.$row['address'].'</h5>';
      $idpref = explode(',',$row['preferences']);
      foreach($idpref as $pref){
      $sql2 = "SELECT * FROM preferences WHERE id='$pref'";
      $res2 = $dbh->query($sql2);
      $res = $res2->fetch();
        echo ' <h5 class="card-title">Preferences: '.$res['name'].'</h5>';
      }
        echo ' <h5 class="card-title">Created at: '.$row['date_start'].','.$row['date_start_h'].'</h5>';
          echo ' <h5 class="card-title">Deadline: '.$row['deadline'].','.$row['deadline_h'].'</h5>';
